...
$.ajax({
    url: '/api/birth-list/',  // Correct API endpoint for registered birth cattle
    method: 'GET',
    dataType: 'json',
    success: function (data) {
        console.log('Birth Data:', data);  // Log the data
...
// Handle form submission for adding a new birth cattle record
document.getElementById('addBirthForm').addEventListener('submit', function(event) {
    event.preventDefault(); 
    console.log('Adding new birth record');
 ...
 // Handle null values
    ...
    .then(({status, data}) => {
        ...
        if (status === 200) {
            ...
            alert('Failed to add new birth record: ' + (data.error || 'Unknown error')); // Notify error
        }
    })
    .catch(error => {
        console.error('Error:', error);
        alert('Failed to add birth record due to an unexpected error.'); // Notify unexpected error
    });
});
